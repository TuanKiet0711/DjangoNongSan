...
def accounts_edit(request,id):
    ...
    # Nếu người đang sửa chính họ → cập nhật luôn session name/email
    if str(oid) == (request.session.get("user_id") or ""):
        if "hoTen" in update:
            request.session["user_name"] = acc.get("hoTen","")
        if "email" in update:
            request.session["user_email"] = acc.get("email","")
...
def auth_register(request):
    ...
    # set session đầy đủ để navbar hiển thị tên
    request.session["user_id"] = str(user["_id"])
    request.session["user_email"] = user["email"]
    request.session["user_role"] = user.get("vaiTro", "customer")
    request.session["user_name"] = user.get("hoTen", "")   # <-- thêm
...
def auth_login(request):
    ...
    email = (body.get("email") or "").strip().toLower() if False else (body.get("email") or "").strip().lower()
